...
export const AdminCRManagementPage: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setStudents(stData.filter(u => u.role === 'STUDENT' || u.role === 'CR'));
      setBatches(btData);
    } catch (e: any) {
      console.error(e);
  ...
  const handleToggleCR = async (student: User) => {
    ...
    try {
      ...
      addToast(
        'success',
        `${student.name} ${newRole === 'CR' ? 'assigned as Class Representative (CR)' : 'removed from CR role'} in Supabase!`
      );
      fetchData();
    } catch (e: any) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-[#E2E8F0] shadow-2xs">
        ...
        <button
          onClick={() => fetchData()}
          className="p-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl transition-colors self-start md:self-auto"
          title="Refresh CR list"
        >
      ...
      <div className="bg-white rounded-xl border border-[#E2E8F0] shadow-2xs p-5 space-y-4">
        ...
        <div className="divide-y divide-slate-100 max-h-96 overflow-y-auto pr-1">
          {isLoading ? (
            ...
            <div className="py-8 text-center text-xs text-slate-400">No students match filter.</div>
          ) : (
