...
=======
      if (editingSlot) {
        ...
        addToast('success', 'Routine class slot updated in Supabase!');
      } else {
        await adminApiClient.createRoutineSlot({
          ...slotForm,
          batchId: selectedBatchId,
        });
        addToast('success', 'New routine class slot added in Supabase!');
      }
      setIsSlotModalOpen(false);
      fetchData();
    } catch (e: any) {
...
=======
      ...
      addToast('success', res.message || `Successfully imported routine slots into Supabase!`);
      setIsJsonModalOpen(false);
      setSelectedBatchId(jsonImportBatchId);
      fetchData();
>>>>>>> ae955ef (Update question bank, exam types and added FAQ)
...
=======
      ...
      {/* Navigation & Batch Selector */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 rounded-xl border border-[#E2E8F0] shadow-2xs">
        <div className="flex items-center gap-2">
          <span className="text-xs font-bold text-slate-700">Select Academic Batch:</span>
          <select
            value={selectedBatchId}
            onChange={e => setSelectedBatchId(e.target.value)}
            className="bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg px-3 py-1.5 text-xs text-slate-900 font-bold focus:ring-2 focus:ring-blue-500"
          ...
          <span className="text-xs text-slate-400 font-medium">({routines.length} Slots Scheduled)</span>
        </div>
        ...
        >
          <RefreshCw className="w-3.5 h-3.5" /> Refresh Routine
        </button>
      </div>
      ...
      <div className="space-y-4">
        {isLoading ? (
          <div className="py-12 text-center text-xs text-slate-400">
            Loading routine for {selectedBatchObj?.name || 'batch'}...
          </div>
        ) : (
          <VisualRoutineGrid
            routines={routines}
